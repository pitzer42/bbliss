	...
	function createStream() {
	  ...
	  localizer(function (location) {
	    ...
	    socket.on('connect', function () {
	      ...
	      var source = new FluxoPeer(servers, socket);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var SignalingChannel = function SignalingChannel(socket) {
	  ...
	  socket.on(SEND_DESCRIPTION, function (origin, target, description) {
	    ...
	    if (target === socket.id) {
	      console.log(typeof description === 'undefined' ? 'undefined' : (0, _typeof3.default)(description));
	      _this.onDescription(origin, description);
	    }
	  });
	...
	var FluxoPeer = function FluxoPeer(servers, socket) {
	  ...
	  this.offer = function (constraints, _title, _location) {
	    title = _title;
	    location = _location;
	    navigator.getUserMedia(constraints, onStreamCaptured, _this2.onError);
	  };

	  var onStreamCaptured = function onStreamCaptured(_stream) {
	    stream = _stream;
	    _this2.displayStream(stream);
	    createConnection(stream);
	  };

	  var createConnection = function createConnection(stream) {
	    if (connectedChildren >= MAX_CHILDREN) {
	      _this2.onError('cannot support more than ' + MAX_CHILDREN + ' connections');
	      return null;
	    }
	    connectedChildren++;
	    ...
	    connection.createOffer(setLocalDescription, _this2.onError);
	  };
	  ...
	  var makeAvailable = function makeAvailable(connection) {
	    return function (event) {
	      if (event.candidate === null) {
	        ...
	        signaling.onDescription = function (origin, description) {
	          connectChild(connection, origin, description);
	        };
	  ...
	  var connectChild = function connectChild(connection, origin, description) {
	  ...
	  this.receive = function (_title, _location) {
	    ...
	    signaling.onDescription = function (target, remoteDescription) {
